...
// 获取 DOM 节点
const title = $('.img-container h3'); // 获取标题
const imgBox = $('.img-box'); // 图片容器
const imgBlock = $('.img-block'); // 可以拖动的图片
const imgGap = $('.img-gap'); // 图片缺口
const slider = $('.slider'); // 滑块条
const btn = $('#btn'); // 拖动的滑块
const span = $('.slider span'); // 滑块条的文字
const imgArr = ['t1.png', 't2.png', 't3.png', 't4.png', 't5.png']; // 存储所有图片的数组
...
function init() {
  ...
  const heightRange = imgBox.offsetHeight - imgBlock.offsetHeight;
  // 空缺图块可以设置的最大 left 值 = 盒子容器的宽度的一半 - 空缺图块的宽度
  const widthRange = imgBox.offsetWidth / 2 - imgBlock.offsetWidth;
  // 生成随机的 left 和 top 值
  const top = Math.random() * heightRange;
  const left = Math.random() * widthRange + imgBox.offsetWidth / 2;
  ...
  btn.onmousedown = function (e) {
    ...
    document.onmouseup = function () {
      // 获取两个图块的 left 差值
      const diffLeft = imgGap.offsetLeft - imgBlock.offsetLeft;
